...
class DecordVideoReader(VideoReader):
    ...
    def __init__(self, path: TYPE_PATH) -> None:
        ...
        try:
            self.video_reader = decord.VideoReader(path, num_threads=8)
    ...
    def time_to_indices(self, time: Union[float, Sequence[float]]) -> np.ndarray:
        len_video = len(self)
        times = self.video_reader.get_frame_timestamp(range(len_video)).mean(-1) if self.video_reader else np.zeros(1)
        indices = np.searchsorted(times, time).clip(0,len_video -1)
        # Use `np.bitwise_or` so it works both with scalars and numpy arrays.
        indices_ = np.where(np.bitwise_or(indices == 0, times[indices] - time <= time - times[indices - 1]), indices, indices - 1)
        return indices_

    @overrides
    def get_frame_timestamp(self, frame_index: int) -> float:
        return self.video_reader.get_frame_timestamp(frame_index)
