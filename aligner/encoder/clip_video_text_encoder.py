...
class ClipVideoTextEncoder(VideoTextEncoder):
    def __init__(self, model: CLIP, num_frames: int = 4,
                 eval_sampler = 'constFPS', target_video_fps=1.0, ) -> None:
        super().__init__()
        self.model = model
    ...
    def encode_video(self, video: TYPE_VIDEO_INPUT) -> torch.Tensor:
        ...
        encoded_video = encoded_video.view(batch_size, -1, *encoded_video.shape[1:])

        # Averaging the representations is the same as averaging the predictions:
        # <t, (i1+i2)/2> = 1/2 <t, i1+i2> = (<t, i1> + <t, i2>) / 2
        # This is averaging through all the frames (in a batch and in all samples of a video)
        return {"encoded_images":encoded_video}
